...
parser.add_argument('--test', default=True, help='test')  # False/True
...
def validate(valid_loader, model):
    ...
    with torch.no_grad():
        for uids, iids, labels, u_itemsdiv, u_avg, u_users, u_users_items, i_users, i_avg in tqdm(valid_loader):
            ...
            #
            # for j in range(10):
            #     print(uids[j],iids[j],labels[j],preds[j])

            for i, uid in enumerate(uids):
                if uid.item() not in result_dict:
                    result_dict[uid.item()] = {}

                if iids[i].item() not in result_dict[uid.item()]:
                    result_dict[uid.item()][iids[i].item()] = preds[i].item()
