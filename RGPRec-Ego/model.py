...
class _DevModel(nn.Module):
    ''' user modeling to learn developer latent factors.
    '''

    def __init__(self, emb_dim, user_emb, item_emb, rate_emb):
        super(_DevModel, self).__init__()
...
class _TaskModel(nn.Module):
    ...
    def __init__(self, emb_dim, user_emb, item_emb, rate_emb):
        super(_TaskModel, self).__init__()
        self.emb_dim = emb_dim
        self.user_emb = user_emb
        self.item_emb = item_emb
        self.rate_emb = rate_emb
...
class RGPRec(nn.Module):
    ...
    def __init__(self, num_users, num_items, num_rate_levels, emb_dim=64):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.eps = 1e-10
        self.user_model = _DevModel(self.emb_dim, self.user_emb, self.item_emb, self.rate_emb)

        self.item_model = _TaskModel(self.emb_dim, self.user_emb, self.item_emb, self.rate_emb)
